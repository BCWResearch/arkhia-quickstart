...
contract FairTradeCoffee is ExpiryHelper {
    ...
    function withdrawDonations() public {
        require(fairTradeMetadata.tokenSupply <= 10000, "We need more donations!");
    ...
    function setCreatorName (string memory _creatorName) public {
        fairTradeMetadata.creatorName  = _creatorName;
    }

    function tokenAssociate(address _account) external returns (int){
        int response = HederaTokenService.associateToken(_account, fairTradeMetadata.tokenAddress);
    ...
    function exchangeFairTradeToken(address receiver, int64 amount) external payable returns(int) {
        int transferResponse = HederaTokenService.transferToken(fairTradeMetadata.tokenAddress, address(this), receiver, amount);
